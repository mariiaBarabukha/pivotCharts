  ...
  export class PivotCore extends apexcharts.Core {
    resizeNonAxisCharts() {
      ...
      if (
        ...
        legendHeight =
          new PivotLegend(this.ctx, {}).legendHelpers.getLegendBBox().clwh + 10;
      }
      ...
      if (gl.comboCharts) {
        ...
        if (columnSeries.series.length > 0) {
          if (w.config.chart.stacked) {
            ...
            elGraph.push(
              this.ctx.bar.draw(columnSeries.series, columnSeries.i)
            );
          }
        }
        ...
        if (boxplotSeries.series.length > 0) {
          elGraph.push(
            boxCandlestick.draw(boxplotSeries.series, boxplotSeries.i)
          );
        }
